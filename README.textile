...
Elasticassandra is a fork of "Elasticsearch":https://github.com/elastic/elasticsearch version 1.5 modified 
to run on top of "Apache Cassandra":http://cassandra.apache.org in a scalable and resilient peer-to-peer architecture. Elasticsearch code is embedded in 
Cassanda nodes providing advanced search features on Cassandra tables and Cassandra serve as an Elasticsearch data and configuration store.
...
* Each node only index local data and act as a primary local shard.
...
* Install Apache Cassandra version 2.18
...
curl -XGET 'http://localhost:9200/twitter/tweet/_search?q=user:kimchy&pretty=true'
{
  ...
  "hits" : {
    ...
    "hits" : [ {
      ...
      "_id" : "1",
      "_score" : 1.0,
      "_source":{"message":"Trying out Elasticsearch, so far so good?","postDate":1258290720000,"user":"kimchy"}
    }, {
      "_index" : "twitter",
      "_type" : "tweet",
      "_id" : "2",
      "_score" : 1.0,
      "_source":{"message":"Another tweet, will it be indexed?","postDate":1258294332000,"user":"kimchy"}
    } ]
  }
}
</pre>
...
Contributors are welcome to test and enhance Elasticassandra to make it a production ready product.
