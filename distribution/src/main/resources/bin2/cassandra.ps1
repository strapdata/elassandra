...
Function Main
{
    ...
    if ($elass)
    {
        echo "Starting with Elasticsearch enabled."
        $env:JVM_OPTS = $env:JVM_OPTS + " -Djava.awt.headless=true"
        $env:CASSANDRA_MAIN="org.apache.cassandra.service.ElassandraDaemon"
    }
    if ($p)
    {
        $pidfile = "$p"
