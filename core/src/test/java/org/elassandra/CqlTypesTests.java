...
public class CqlTypesTests extends ESSingleNodeTestCase {
    ...
    public void testAllTypesTest() throws Exception {
        ...
        assertThat(client().prepareIndex("ks1", "natives", "1")
                .setSource("{\"c2\": \"toto\", \"c3\" : \"2016-10-10\", \"c4\": 1, \"c5\":44, \"c6\":1.0, \"c7\":2.22, \"c8\": true, \"c9\":\"U29tZSBiaW5hcnkgYmxvYg==\", \"c10\":\"ae8c9260-dd02-11e6-b9d5-bbfb41c263ba\",\"c11\":\"ae8c9260-dd02-11e6-b9d5-bbfb41c263ba\", \"c12\":1, \"c13\":1  }")
    ...
    public void testCompoundPkTypesTest() throws Exception {
        ...
        for(int i=0; i < types.length; i++) {
            ...
            process(ConsistencyLevel.ONE,String.format(Locale.ROOT,"CREATE TABLE ks2.t%s (pk%s %s, ck %s, v %s, PRIMARY KEY (pk%s,ck))", name, name, type, types[randomCk], types[randomVal], name));
            process(ConsistencyLevel.ONE,String.format(Locale.ROOT,"INSERT INTO ks2.t%s (pk%s, ck, v) VALUES (?, ?, ?)", name, name), values[i], values[randomCk], values[randomVal]);
        }
        ...
        for(int i=0; i < types.length; i++) {
            String type = types[i];
            String name = names[i];
            String mapping = String.format(Locale.ROOT,"{ \"t%s\" : { \"discover\" : \"^((?!timeuuid2).*)\", \"properties\":{ \"timeuuid2\":{ \"type\":\"date\" }}}}", name);
            System.out.println("discover pk name="+name+" type="+type+" mapping="+mapping);
            assertAcked(client().admin().indices()
        ...
        for(int i=0; i < types.length; i++) {
            String type = types[i];
            String name = names[i];
            System.out.println("search pk name="+name+" type="+type);
            assertThat(client().prepareSearch()
                    .setIndices("ks2")
    ...
    public void testTextGeohashMapping() throws Exception {
        ...
        ByteBuffer[] elements = new ByteBuffer[] {
                ...
                ClusterService.serialize("test", "geoloc", DoubleType.instance, GeoPointFieldMapper.Names.LON, 29.411767, null)
        };
    ...
    public void testUUID() throws Exception {
        ...
        assertThat(client().prepareIndex("test", "pk_uuid", "bacc6c75-91b8-4a86-a408-ff7bafac535d").setSource("{ \"column_not_uuid\": \"a value\" }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "pk_uuid", "bacc6c75-91b8-4a86-a408-ff7bafac535d").setSource("{ \"column_not_uuid\": \"a value\", \"pk_uuid\": \"bacc6c75-91b8-4a86-a408-ff7bafac535d\" }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "pk_not_uuid", "pk2").setSource("{ \"column_uuid\": \"bacc6c75-91b8-4a86-a408-ff7bafac535d\" }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "pk_not_uuid", "pk2").setSource("{ \"column_uuid\": \"bacc6c75-91b8-4a86-a408-ff7bafac535d\", \"pk_not_uuid\":\"pk2\" }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
    }
    ...
    public void testNullValue() throws Exception {
        ...
        assertThat(client().prepareIndex("test", "my_type", "1").setSource("{\"status_code\": \"OK\" }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "my_type", "2").setSource("{\"status_code\": [ \"NOK\", \"OK\" ] }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "my_type", "3").setSource("{\"status_code\": null }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
        assertThat(client().prepareIndex("test", "my_type", "4").setSource("{\"status_code\": [] }").get().getResult(), equalTo(DocWriteResponse.Result.CREATED));
