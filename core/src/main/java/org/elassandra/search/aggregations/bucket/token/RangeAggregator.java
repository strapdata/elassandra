...
public class RangeAggregator extends BucketsAggregator {
    ...
    public static class Range implements Writeable, ToXContent {
        ...
        protected Range(String key, Long from, String fromAsStr, Long to, String toAsStr) {
            this.key = key;
            this.from = from == null ? Long.MAX_VALUE : from;
            this.fromAsStr = fromAsStr;
            this.to = to == null ? Long.MIN_VALUE : to;
        ...
        public Range process(DocValueFormat parser, SearchContext context) {
            ...
            Long to = this.to;
            if (fromAsStr != null) {
                from = parser.parseLong(fromAsStr, false, context.getQueryShardContext()::nowInMillis);
            }
            if (toAsStr != null) {
                to = parser.parseLong(toAsStr, false, context.getQueryShardContext()::nowInMillis);
            }
