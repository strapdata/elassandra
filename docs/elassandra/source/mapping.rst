...
|                             |                                  | Note that fields in the partition key are counted separately than other fields in the primary key.                                                                                                                                                  |
|                             |                                  | For example, for a composite primary key such as **PRIMARY KEY (user, post_date, id)**, the *user* column should have a *cql_primary_key_order* of **0** (being the first partition key).                                                           | 
|                             |                                  | *post_date* should also have a *cql_primary_key_order* of **0** (being the first clustering key), and *id* should have a *cql_primary_key_order* of **1** (being the second clustering key).                                                        |  
|                             |                                  | You can also check to see what numbers to use for your *cql_primary_key_order* by checking your schema using CQL: **SELECT * FROM system_schema.columns WHERE keyspace_name = 'my_keyspace' AND table_name = 'my_table';**                          |
+-----------------------------+----------------------------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
