...
if [[ $($CASSANDRA_HOME/bin/nodetool status 2>/dev/null | grep "$POD_IP" | awk '{ print $1 }') == "UN" ]]; then
  if [[ "$CASSANDRA_DAEMON" == "org.apache.cassandra.service.CassandraDaemon" ]] || \
     [[ $(curl -XGET ${ELASSANDRA_MONITOR_PASSWORD:+"--user"} ${ELASSANDRA_MONITOR_PASSWORD:+"monitor:$ELASSANDRA_MONITOR_PASSWORD"} "${PROTOCOL:-http}://localhost:9200/_cat/nodes?h=ip" 2>/dev/null | grep "$POD_IP" ) == "$POD_IP" ]]; then
     exit 0;
  else 
     if [[ $DEBUG ]]; then
