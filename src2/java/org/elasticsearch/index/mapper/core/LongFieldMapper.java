...
public class LongFieldMapper extends NumberFieldMapper {
    ...
    protected void createField(ParseContext context, List<Field> fields, Object value) throws IOException {
        ...
        if (context.includeInAll(includeInAll, this)) {
            context.allEntries().addText(fieldType().names().fullName(), Long.toString((Long)value), boost);
        }
        if (fieldType().indexOptions() != IndexOptions.NONE || fieldType().stored()) {
            CustomLongNumericField field = new CustomLongNumericField((Long)value, fieldType());
            field.setBoost(boost);
            fields.add(field);
        }
        if (fieldType().hasDocValues()) {
            addDocValue(context, fields, (Long)value);
        }
    }
