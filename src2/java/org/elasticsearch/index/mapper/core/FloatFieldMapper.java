...
public class FloatFieldMapper extends NumberFieldMapper {
    ...
    @Override
    protected void createField(ParseContext context, List<Field> fields, Object value) throws IOException {
        float boost = fieldType().boost();
        if (value == null) {
            if (fieldType().nullValue() == null) {
                return;
        ...
        if (context.includeInAll(includeInAll, this)) {
            context.allEntries().addText(fieldType().names().fullName(), Float.toString((Float)value), boost);
        }
        if (fieldType().indexOptions() != IndexOptions.NONE || fieldType().stored()) {
            CustomFloatNumericField field = new CustomFloatNumericField((Float)value, fieldType());
            field.setBoost(boost);
            fields.add(field);
        }
        if (fieldType().hasDocValues()) {
            if (useSortedNumericDocValues) {
                addDocValue(context, fields, floatToSortableInt((Float)value));
            } else {
                CustomFloatNumericDocValuesField field = (CustomFloatNumericDocValuesField) context.doc().getByKey(fieldType().names().indexName());
                if (field != null) {
                    field.add((Float)value);
                } else {
                    field = new CustomFloatNumericDocValuesField(fieldType().names().indexName(), (Float)value);
                    context.doc().addWithKey(fieldType().names().indexName(), field);
                }
            }
        }
