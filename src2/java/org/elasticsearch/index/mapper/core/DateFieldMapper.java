...
import org.elasticsearch.index.mapper.core.DoubleFieldMapper.CustomDoubleNumericDocValuesField;
...
public class DateFieldMapper extends NumberFieldMapper {
    ...
    @Override
    protected void createField(ParseContext context, List<Field> fields, Object _value) throws IOException {
        float boost = fieldType().boost();
        if (_value == null) {
            if (fieldType().nullValue() == null) {
                return;
            }
            _value = fieldType().nullValue();
        }
        if (context.includeInAll(includeInAll, this)) {
            context.allEntries().addText(fieldType().names().fullName(), Double.toString(value), boost);
        }
        Long value = ((Date) _value).getTime();
        if (value != null) {
            if (fieldType().indexOptions() != IndexOptions.NONE || fieldType().stored()) {
                CustomLongNumericField field = new CustomLongNumericField(value, fieldType());
                field.setBoost(boost);
