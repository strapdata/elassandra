...
public class ShortFieldMapper extends NumberFieldMapper {
    ...
    protected void createField(ParseContext context, List<Field> fields, Object value) throws IOException {
        ...
        if (context.includeInAll(includeInAll, this)) {
            context.allEntries().addText(fieldType().names().fullName(), Short.toString((Short)value), boost);
        }
        if (fieldType().indexOptions() != IndexOptions.NONE || fieldType().stored()) {
            CustomShortNumericField field = new CustomShortNumericField((Short)value, fieldType());
            field.setBoost(boost);
            fields.add(field);
        }
        if (fieldType().hasDocValues()) {
            addDocValue(context, fields, (Short)value);
        }
    }
