 ...
 */
package org.elasticsearch.search.aggregations.bucket.token;
...
public class RangeAggregator extends BucketsAggregator {
    ...
    public static class Range {
        ...
        public Range(String key, long from, String fromAsStr, long to, String toAsStr) {
            this.key = key;
            this.from = from;
        ...
        public void process(ValueParser parser, SearchContext context) {
            assert parser != null;
            if (fromAsStr != null) {
                from = parser.parseLong(fromAsStr, context);
            }
            if (toAsStr != null) {
                to = parser.parseLong(toAsStr, context);
            }
        }
    }
    ...
    public RangeAggregator(String name, AggregatorFactories factories, ValuesSource.Numeric valuesSource, ValueFormat format,
        ...
        maxTo = new long[this.ranges.length];
    ...
    private int collect(int doc, long value, long owningBucketOrdinal, int lowBound) throws IOException {
    ...
    public InternalAggregation buildAggregation(long owningBucketOrdinal) throws IOException {
        List<org.elasticsearch.search.aggregations.bucket.token.Range.Bucket> buckets = new ArrayList<>(ranges.length);
        for (int i = 0; i < ranges.length; i++) {
            Range range = ranges[i];
            final long bucketOrd = subBucketOrdinal(owningBucketOrdinal, i);
            org.elasticsearch.search.aggregations.bucket.token.Range.Bucket bucket =
    ...
    public static class Unmapped extends NonCollectingAggregator {
        ...
        public InternalAggregation buildEmptyAggregation() {
            InternalAggregations subAggs = buildEmptySubAggregations();
            List<org.elasticsearch.search.aggregations.bucket.token.Range.Bucket> buckets = new ArrayList<>(ranges.size());
