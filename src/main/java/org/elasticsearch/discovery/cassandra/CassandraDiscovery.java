...
public class CassandraDiscovery extends AbstractLifecycleComponent<Discovery> implements Discovery, IEndpointStateChangeSubscriber {
    ...
    @Inject
    public CassandraDiscovery(Settings settings, ClusterName clusterName, TransportService transportService, ClusterService clusterService,
                          DiscoveryNodeService discoveryNodeService, Version version, DiscoverySettings discoverySettings, SchemaService schemaService) {
        super(settings);
    ...
    public static String buildNodeName(InetAddress addr) {
    	...
    	return String.format("node%03d%03d%03d%03d", 
    			(int) (addr.getAddress()[0] & 0xFF), 
    			(int) (addr.getAddress()[1] & 0xFF), 
    			(int) (addr.getAddress()[2] & 0xFF), 
    			(int) (addr.getAddress()[3] & 0xFF));
    }
    ...
    protected void doStart() throws ElasticsearchException {
        ...
        synchronized (clusterGroups) {
            ...
            // get local node from cassandra cluster
            {
                ...
                localNode = new DiscoveryNode(buildNodeName(localAddress), 
                		hostId,  
            ...
            for(Entry<InetAddress,UUID> entry : peers.entrySet()) {
            	if ( (!entry.getKey().equals(localAddress)) && ( localDc.equals(endpointInfo.get(entry.getKey()).get("data_center")) ) )  {
            		...
            		DiscoveryNode dn = new DiscoveryNode(buildNodeName(entry.getKey()), 
                    		entry.getValue().toString(),  
                    		new InetSocketTransportAddress(entry.getKey(), settings.getAsInt("transport.tcp.port", 9300)),
                    		attrs, 
                            version);
    ...
    private void updateClusterState(String source, MetaData newMetadata1) {
    	...
    	clusterService.submitStateUpdateTask(source, new ProcessedClusterStateNonMasterUpdateTask() {
            ...
            public CassandraClusterState execute(CassandraClusterState currentState) {
            	CassandraClusterState.Builder newStateBuilder = CassandraClusterState.builder(currentState);
            	...
            	CassandraClusterState newClusterState = newStateBuilder.build();
            	...
            	if (newRoutingTable != currentState.routingTable()) {
            		CassandraClusterState newClusterState2 = CassandraClusterState.builder(newClusterState)
        	...
        	if (DatabaseDescriptor.getEndpointSnitch().getDatacenter(entry.getKey()).equals(localDc)) {
        			...
        			if (vv != null) {
	        			...
	        			if (dn == null)  {
		                    ...
		                    dn = new DiscoveryNode(
				        			buildNodeName(entry.getKey()), 
				        			hostId.toString(),  
			                		new InetSocketTransportAddress(entry.getKey(), settings.getAsInt("transport.tcp.port", 9300)),
			                		attrs, version);
		                    dn.status( status );
    ...
    public MetaData hasNewMetaData() {
    	...
    	if (!newMetaData.uuid().equals(currentMetaData.uuid()) || (newMetaData.version() != currentMetaData.version())) {
    		logger.debug("updating metadata from uid/version={}/{} to {}/{}",
    				currentMetaData.uuid(),currentMetaData.version(), 
    				newMetaData.uuid(),newMetaData.version());
    		return newMetaData;
    	}
    ...
    public void updateNode(InetAddress addr, EndpointState state) {
    	...
    	if (DatabaseDescriptor.getEndpointSnitch().getDatacenter(addr).equals(localDc)) {
			...
			if (dn == null)  {
                ...
                dn = new DiscoveryNode(
	        			buildNodeName(addr), 
	        			hostId.toString(),  
                		new InetSocketTransportAddress(addr, settings.getAsInt("transport.tcp.port", 9300)),
                		attrs, version);
                dn.status( status );
    	...
    	if (updatedNode) updateClusterState("update-node-"+addr.getHostAddress(),null);
	}
     ...
     */
	@Override
    public synchronized void writeIndexShardSate(String index, ShardRoutingState shardRoutingState) throws JsonGenerationException, JsonMappingException, IOException {
...
/*
    private void publish(DiscoveryNode[] members, CassandraClusterState clusterState, final BlockingClusterStatePublishResponseHandler publishResponseHandler) {
        ...
        try {
            ...
            for (final DiscoveryNode node : members) {
                ...
                final CassandraClusterState nodeSpecificClusterState = CassandraClusterState.Builder.fromBytes(clusterStateBytes, node, clusterName);
                nodeSpecificClusterState.status(CassandraClusterState.ClusterStateStatus.RECEIVED);
                ...
                if (nodeSpecificClusterState.nodes().localNode() != null) {
                    ...
                    this.clusterService.submitStateUpdateTask("cassandra-disco-receive(from "+this.nodeName()+")", new ProcessedClusterStateNonMasterUpdateTask() {
                        ...
                        public CassandraClusterState execute(CassandraClusterState currentState) {
                            ...
                            CassandraClusterState.Builder builder = CassandraClusterState.builder(nodeSpecificClusterState);
...
/*
    private class NewClusterStateListener implements PublishClusterStateAction.NewClusterStateListener {
        ...
        @Override
        public void onNewClusterState(CassandraClusterState clusterState, NewStateProcessed newStateProcessed) {
            handleNewClusterStateFromMaster(clusterState, newStateProcessed);
        }
    }
    
    static class ProcessClusterState {
        final CassandraClusterState clusterState;
        final PublishClusterStateAction.NewClusterStateListener.NewStateProcessed newStateProcessed;
        volatile boolean processed;

        ProcessClusterState(CassandraClusterState clusterState, PublishClusterStateAction.NewClusterStateListener.NewStateProcessed newStateProcessed) {
    ...
    void handleNewClusterStateFromMaster(CassandraClusterState newClusterState, final PublishClusterStateAction.NewClusterStateListener.NewStateProcessed newStateProcessed) {
        ...
        final CassandraClusterState newState = newClusterState;
        ...
        clusterService.submitStateUpdateTask("cassandra-disco-receive_cluster_state_from_another_node [" + newState.nodes().localNodeId() + "]", Priority.URGENT, new ProcessedClusterStateUpdateTask() {
            ...
            public CassandraClusterState execute(CassandraClusterState currentState) {
            	...
            	CassandraClusterState mergedClusterState = new CassandraClusterState.Builder(currentState)
            	...
            	CassandraClusterState newClusterState = new CassandraClusterState.Builder(mergedClusterState)
