...
public class ElasticSecondaryIndex extends PerRowSecondaryIndex {
	...
	/**
	 * Index a mutation.
	 * Set empty field for deleted cells.
	 */
	@Override
	...
	private void synchronousIndex(String indexName, String type, String id, Object[] sourceData, long ttl, boolean refresh) throws Throwable {
	    ...
	    SourceToParse sourceToParse = SourceToParse.source(SourceToParse.Origin.PRIMARY, source(sourceData))
	    		.type(type)
	    		.id(id)
	    ...
	    try {
	        ...
	        if (index.parsedDoc().mappingsModified()) {
            	// mapping is done via putMapping or when indexing a document (see index action)
            }
            */
		...
		for(String target : cdef.getIndexOptions().get(ELASTIC_OPTION_TARGETS).split(",")) {
			...
			if ((indexAndType != null)&&(indexAndType.length==2)) {
				...
				if (!this.baseCfs.keyspace.getName().equals(targetPair.left) || !this.baseCfs.name.equals(targetPair.right)) {
					// specific maping from Elastic index.type to Cassandra keyspace.table
	...
	public void validateOptions() throws ConfigurationException {
		for(ColumnDefinition cd : getColumnDefs()) {
			for(String optionKey : cd.getIndexOptions().keySet()) {
				if (!(optionKey.equals(ELASTIC_OPTION_TARGETS) ||
					  optionKey.equals(CUSTOM_INDEX_OPTION_NAME))) {
	...
	public boolean indexes(CellName name) {
		...
		if ((cdef != null) && 
			(cdef.getIndexOptions() != null) && 
			(this.getClass().getCanonicalName().equals(cdef.getIndexOptions().get(CUSTOM_INDEX_OPTION_NAME)))) {
			return true;
		}
		/*
		 * ..However, some type of COMPACT STORAGE layout do not
		 * store the CQL3 column name in the cell name and so this part can be null => return true
		 */
