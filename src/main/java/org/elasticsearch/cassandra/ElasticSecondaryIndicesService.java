...
import org.elasticsearch.cassandra.SecondaryIndicesService.DeleteListener;
...
public class ElasticSecondaryIndicesService extends AbstractLifecycleComponent<SecondaryIndicesService> implements SecondaryIndicesService {
    ...
    @Inject
    public ElasticSecondaryIndicesService(Settings settings,  SchemaService schemaService,
    ...
    class CreateSecondaryIndexTask extends Task {
        ...
        public void execute() {
            try {
                ...
                schemaService.createSecondaryIndices(ksName);
            } catch (IOException e) {
    ...
    class DropSecondaryIndexTask extends Task {
        ...
        public void execute() {
            ...
            for(Iterator<IndexService> it = indicesService.iterator(); it.hasNext(); ) {
                ...
                if (ksName.equals(indexService2.getIndexSettings().get(IndexMetaData.SETTING_KEYSPACE_NAME, indexService2.index().name()))) {
            ...
            if (isLastRemainingIndex) {
                try {
                    ...
                    schemaService.dropSecondaryIndices(ksName);
    ...
    protected void doStart() throws ElasticsearchException {
        ...
        this.tasksExecutor = EsExecutors.newSinglePrioritizing(daemonThreadFactory(settings, TASK_THREAD_NAME));
    ...
    public void dropSecondaryIndices(String index) {
        ...
        String ksName = indexService.getIndexSettings().get(IndexMetaData.SETTING_KEYSPACE_NAME, indexService.index().name() );
    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        if (event.metaDataChanged()) {
            for(Iterator<String> it = event.state().metaData().indices().keysIt(); it.hasNext(); ) {
                ...
                if (event.indexMetaDataChanged(event.state().metaData().index(index)) && event.state().metaData().index(index).mappings().size() > 0) {
