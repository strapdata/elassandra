...
public class CassandraGateway extends AbstractLifecycleComponent<Gateway> implements Gateway, ClusterStateListener {
    ...
    @Inject
    public CassandraGateway(Settings settings, ClusterName clusterName, ClusterService clusterService, 
    		NodeEnvironment nodeEnv,CassandraDiscovery cassandraDiscovery, SchemaService elasticSchemaService) {
        super(settings);
    ...
    public void performStateRecovery(final GatewayStateRecoveredListener listener) throws GatewayException {
        ...
        CassandraClusterState.Builder builder = CassandraClusterState.builder(clusterService.state());
    ...
    public void clusterChanged(final ClusterChangedEvent event) {
    	/*
    	final CassandraClusterState newState = event.state();
    	...
    	logger.debug("clusterChanged metaDataChange={} source={} old_metadata={}/{} new_metadata={}/{} ", 
    			event.metaDataChanged(), event.source(), 
    			event.previousState().metaData().uuid(), event.previousState().metaData().version(), 
    			newMetaData.uuid(), newMetaData.version()  );
    	

    	CassandraClusterState previousState = event.previousState();
