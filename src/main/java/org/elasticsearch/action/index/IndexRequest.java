 ...
 * it searchable. Best created using
 * {@link org.elasticsearch.client.Requests#indexRequest(String)}.
 * <p/>
 * <p>
 * The index requires the {@link #index()}, {@link #type(String)},
 * {@link #id(String)} and {@link #source(byte[])} to be set.
 * <p/>
 * <p>
 * The source (content to index) can be set in its bytes form using (
 * {@link #source(byte[])}), its string form ({@link #source(String)}) or using
 * a {@link org.elasticsearch.common.xcontent.XContentBuilder} (
...
public class IndexRequest extends ShardReplicationOperationRequest<IndexRequest> implements DocumentRequest<IndexRequest> {
    ...
    public static enum OpType {
        /**
         * Insert the source. If there an existing document with the id, it will
         * be replaced.
         */
        INDEX((byte) 0),
    ...
    /**
     * Creates an index request caused by some other request, which is provided
     * as an argument so that its headers and context can be copied to the new
     * request
     */
    ...
    /**
     * Copy constructor that creates a new index request that is a copy of the
     ...
     * {@link #type(String)} {@link #source(byte[])} must be set.
     */
    ...
    /**
     * Constructs a new index request against the index, type, id and using the
     * source.
     *
     * @param index
    ...
    /**
     * Before we fork on a local thread, make sure we copy over the bytes if
     * they are unsafe
     */
    @Override
    ...
    /**
     * Sets the content type that will be used when generating a document from
     * user provided objects (like Map).
     */
    ...
    /**
     * Sets the id of the indexed document. If not set, will be automatically
     * generated.
     */
     ...
     * {@link #opType(org.elasticsearch.action.index.IndexRequest.OpType)}. Can
     * be either "insert" or "insert_if_not_exists".
     */
    ...
    /**
     * Set to <tt>true</tt> to force this index to use {@link OpType#INDEX}.
     */
    ...
    /**
     * Sets the parent id of this document. If routing is not set, automatically
     * set it as the routing as well.
     */
    ...
    /**
     * Sets the timestamp either as millis since the epoch, or, in the
    ...
    /**
     * Sets the relative ttl value. It musts be > 0 as it makes little sense
     * otherwise. Setting it to <tt>null</tt> will reset to have no ttl.
     */
    ...
    /**
     * Index the Map as a
     * {@link org.elasticsearch.client.Requests#INDEX_CONTENT_TYPE}.
     *
     * @param source
     *            The map to index
     */
     ...
     * <p/>
     * <p>
     * Note, its preferable to either set it using
     * {@link #source(org.elasticsearch.common.xcontent.XContentBuilder)} or
     * using the {@link #source(byte[])}.
     */
    ...
    /**
     * Sets the document to index in bytes form (assumed to be safe to be used
     ...
     *            The length of the data
     * @param unsafe
     *            Is the byte array safe to be used form a different thread
     */
    ...
    /**
     * Should a refresh be executed post this index operation causing the
     * operation to be searchable. Note, heavy indexing should not set this to
    ...
    /**
     * Sets the version, which will cause the index operation to only be
     * performed if a matching version exists and no changes happened on the doc
     * since then.
     */
    ...
    public void process(MetaData metaData, @Nullable MappingMetaData mappingMd, boolean allowIdGeneration, String concreteIndex) throws ElasticsearchException {
        ...
        // generate timestamp if not provided, we always have one post this
        // stage...
        if (timestamp == null) {
    ...
    public String toString() {
        ...
        return "upsert {[" + index + "][" + type + "][" + id + "], source[" + sSource + "]}";
    }
}
