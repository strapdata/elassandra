...
public class AllocationService extends AbstractComponent {
    ...
    @Inject
    public AllocationService(Settings settings, ClusterService clusterService, AllocationDeciders allocationDeciders, ShardsAllocators shardsAllocators) {
        super(settings);
    ...
    public RoutingAllocation.Result applyFailedShards(ClusterState clusterState, List<ShardRouting> failedShards) {
        ...
        // shuffle the unassigned nodes, just so we won't have things like poison failed shards
        routingNodes.unassigned().shuffle();
        FailedRerouteAllocation allocation = new FailedRerouteAllocation(allocationDeciders, routingNodes, clusterState.nodes(), failedShards, new ClusterInfo());
    ...
    public RoutingAllocation.Result reroute(ClusterState clusterState, AllocationCommands commands, boolean explain) throws ElasticsearchException {
        ...
        RoutingAllocation allocation = new RoutingAllocation(allocationDeciders, routingNodes, clusterState.nodes(), new ClusterInfo());
    ...
    // assign 
    private boolean assignLocalPrimaryShard(RoutingAllocation allocation) {
        boolean changed = false;
