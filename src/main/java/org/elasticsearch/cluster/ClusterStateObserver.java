    ...
    public void waitForNextChange(Listener listener, ChangePredicate changePredicate, @Nullable TimeValue timeOutValue) {
        ...
        if (timeOutValue == null) {
            ...
            if (timeoutTimeLeft <= 0l) {
                ...
                logger.debug("observer[{}] timed out. notifying listener. timeout setting [{}], time since start [{}]", source, timeOutValue, new TimeValue(timeSinceStart));
        ...
        if (changePredicate.apply(lastState.clusterState, lastState.status, newState.clusterState, newState.status)) {
            ...
            logger.trace("observer[{}]: sampled state accepted by predicate ({})", source, newState);
            lastObservedState.set(newState);
            listener.onNewClusterState(newState.clusterState);
        } else {
            logger.trace("observer[{}]: sampled state rejected by predicate ({}). adding listener to ClusterService", source, newState);
    ...
    class ObserverClusterStateListener implements TimeoutClusterStateListener {
        ...
        public void clusterChanged(ClusterChangedEvent event) {
            ...
            if (context.changePredicate.apply(event)) {
                if (observingContext.compareAndSet(context, null)) {
                    ...
                    logger.trace("observer[{}]: accepting cluster state change ({})", source, state);
                    lastObservedState.set(state);
                    context.listener.onNewClusterState(state.clusterState);
                } else {
                    logger.trace("observer[{}]: predicate approved change but observing context has changed - ignoring (new cluster state version [{}])", source, event.state().version());
                }
            } else {
                logger.trace("observer[{}]: predicate rejected change (new cluster state version [{}])", source, event.state().version());
            }
        }
        ...
        public void postAdded() {
            ...
            if (context.changePredicate.apply(lastState.clusterState, lastState.status, newState.clusterState, newState.status)) {
                ...
                if (observingContext.compareAndSet(context, null)) {
                    logger.trace("observer[{}]: post adding listener: accepting current cluster state ({})", source, newState);
                    clusterService.remove(this);
                    lastObservedState.set(newState);
                    context.listener.onNewClusterState(newState.clusterState);
                } else {
                    logger.trace("observer[{}]: postAdded - predicate approved state but observing context has changed - ignoring ({})", source, newState);
                }
            } else {
                logger.trace("observer[{}]: postAdded - predicate rejected state ({})", source, newState);
            }
        }
