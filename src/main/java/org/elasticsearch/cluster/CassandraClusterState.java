...
/**
 * Cluster state updated by cassandra.
 * 
 * @author vroyer
 *
 */
public class CassandraClusterState implements ToXContent {

	public static enum ClusterStateStatus {
		UNKNOWN((byte) 0), RECEIVED((byte) 1), BEING_APPLIED((byte) 2), APPLIED(
				(byte) 3);
	...
	public interface Custom {
		...
		interface Factory<T extends Custom> {
			...
			void toXContent(T customState, XContentBuilder builder,
	...
	public static <T extends Custom> Custom.Factory<T> lookupFactorySafe(
			String type) throws ElasticsearchIllegalArgumentException {
		Custom.Factory<T> factory = customFactories.get(type);
		if (factory == null) {
			throw new ElasticsearchIllegalArgumentException(
					"No custom state factory registered for type [" + type
							+ "]");
		}
		return factory;
	}

	private static final Logger logger = LoggerFactory.getLogger(CassandraClusterState.class);
	...
	volatile ClusterStateStatus status;
	
	
	public CassandraClusterState(long version, CassandraClusterState state) {
		this(state.clusterName, version, state.metaData(), state.routingTable(), state.nodes(), state.blocks(), state.customs());
	}

	public CassandraClusterState(ClusterName clusterName, long version, MetaData metaData, RoutingTable routingTable, DiscoveryNodes nodes, ClusterBlocks blocks, ImmutableOpenMap<String, Custom> customs) {
	...
	public CassandraClusterState status(ClusterStateStatus newStatus) {
	...
	public CassandraClusterState settingsFilter(SettingsFilter settingsFilter) {
	...
	public String prettyPrint() {
		...
		sb.append("version: ").append(version).append("\n");
		sb.append("meta data version: ").append(metaData.version())
				.append("\n");
		sb.append(nodes().prettyPrint());
		sb.append(routingTable().prettyPrint());
		sb.append(readOnlyRoutingNodes().prettyPrint());
		sb.append(customs.toString());
		*/
		return toString();
	}
	...
	public enum Metric {
		VERSION("version"), MASTER_NODE("master_node"), BLOCKS("blocks"), NODES(
				"nodes"), METADATA("metadata"), ROUTING_TABLE("routing_table"), CUSTOMS(
				"customs");
	...
	public XContentBuilder toXContent(XContentBuilder builder, Params params)
		...
		if (metrics.contains(Metric.NODES)) {
			...
			for (DiscoveryNode node : nodes()) {
				...
				builder.field("status", node.getStatus().toString());
		...
		if (metrics.contains(Metric.METADATA)) {
			...
			for (ObjectCursor<IndexTemplateMetaData> cursor : metaData()
					.templates().values()) {
				...
				for (ObjectObjectCursor<String, CompressedString> cursor1 : templateMetaData
						.mappings()) {
					byte[] mappingSource = cursor1.value.uncompressed();
					XContentParser parser = XContentFactory.xContent(
							mappingSource).createParser(mappingSource);
			...
			for (IndexMetaData indexMetaData : metaData()) {
				...
				builder.startObject("mappings");
				for (ObjectObjectCursor<String, MappingMetaData> cursor : indexMetaData
						.mappings()) {
				...
				for (ObjectCursor<String> cursor : indexMetaData.aliases()
						.keys()) {
			...
			for (ObjectObjectCursor<String, MetaData.Custom> cursor : metaData
					.customs()) {
				builder.startObject(cursor.key);
				MetaData.lookupFactorySafe(cursor.key).toXContent(cursor.value,
						builder, params);
				builder.endObject();
			}
	...
	public static Builder builder(CassandraClusterState state) {
	...
	public static class Builder {
		...
		public Builder(CassandraClusterState state) {
			...
			this.customs = (state.customs()==null) ? ImmutableOpenMap.<String, Custom>builder() : ImmutableOpenMap.builder(state.customs());
		}
		...
		public CassandraClusterState build() {
			return new CassandraClusterState(clusterName, version, metaData, routingTable,
					nodes, blocks, customs.build());
		}

		public static byte[] toBytes(CassandraClusterState state)
				throws IOException {
		 ...
		 *            input bytes
		 * @param localNode
		 *            used to set the local node in the cluster state.
		 * @param defaultClusterName
		 *            this cluster name will be used of if the deserialized
		 *            cluster state does not have a name set (which is only
		 *            introduced in version 1.1.1)
		 */
		public static CassandraClusterState fromBytes(byte[] data,
				DiscoveryNode localNode, ClusterName defaultClusterName)
				throws IOException {
			return readFrom(new BytesStreamInput(data, false), localNode,
					defaultClusterName);
		}

		public static void writeTo(CassandraClusterState state, StreamOutput out)
				throws IOException {
		 ...
		 *            input stream
		 * @param localNode
		 *            used to set the local node in the cluster state. can be
		 *            null.
		 * @param defaultClusterName
		 *            this cluster name will be used of receiving a cluster
		 *            state from a node on version older than 1.1.1 or if the
		 *            sending node did not set a cluster name
		 */
		public static CassandraClusterState readFrom(StreamInput in,
				@Nullable DiscoveryNode localNode,
				@Nullable ClusterName defaultClusterName) throws IOException {
			ClusterName clusterName = defaultClusterName;
			if (in.getVersion().onOrAfter(Version.V_1_1_1)) {
				// it might be null even if it comes from a >= 1.1.1 node since
				// it's origin might be an older node
				if (in.readBoolean()) {
			...
			if (in.getVersion().before(Version.V_1_1_0)) {
				// Ignore the explanation read, since after 1.1.0 it's not part
			...
			for (int i = 0; i < customSize; i++) {
				...
				Custom customIndexMetaData = lookupFactorySafe(type).readFrom(
						in);
