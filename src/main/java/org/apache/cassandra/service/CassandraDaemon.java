...
public class CassandraDaemon {
     ...
     * launched. Date roughly taken from
     * http://perspectives.mvdirona.com/2008/07
     * /12/FacebookReleasesCassandraAsOpenSource.aspx We use this to ensure the
     * system clock is at least somewhat correct at startup.
     */
    ...
    protected void setup() {
        ...
        // log warnings for different kinds of sub-optimal JVMs. tldr use 64-bit
        // Oracle >= 1.6u32
        ...
        /*
         * else { String[] java_version = javaVersion.split("_"); String
         * java_major = java_version[0]; int java_minor; try { java_minor =
         * (java_version.length > 1) ? Integer.parseInt(java_version[1]) : 0; }
         * catch (NumberFormatException e) { // have only seen this with java7
         * so far but no doubt there are other ways to break this
         * logger.info("Unable to parse java version {}",
         * Arrays.toString(java_version)); java_minor = 32; } }
         */
        ...
        // check all directories(data, commitlog, saved cache) for existence and
        // permission
        Iterable<String> dirs = Iterables.concat(Arrays.asList(DatabaseDescriptor.getAllDataFileLocations()),
                Arrays.asList(DatabaseDescriptor.getCommitLogLocation(), DatabaseDescriptor.getSavedCachesLocation()));
        ...
        // check the system keyspace to keep user from shooting self in foot by
        // changing partitioner, cluster name, etc.
        // we do a one-off scrub of the system keyspace first; we can't load the
        // list of the rest of the keyspaces,
    ...
    /**
     * Start the Cassandra Daemon, assuming that it has already been initialized
     * via {@link #init(String[])}
     *
     * Hook for JSVC
     */
    public void start() {
    ...
    public void stop() {
        ...
        // On windows, we need to stop the entire system as prunsrv doesn't have
        // the jsvc hooks
    ...
    private void waitForGossipToSettle() {
        ...
        while (numOkay < GOSSIP_SETTLE_POLL_SUCCESSES_REQUIRED) {
            ...
            if (forceAfter > 0 && totalPolls > forceAfter) {
                logger.warn("Gossip not settled but startup forced by cassandra.skip_wait_for_gossip_to_settle. Gossip Stage total/active/pending/completed: {}/{}/{}/{}", totalPolls, active, pending,
                        completed);
                break;
            }
        }
    ...
    public interface Server {
        /**
         * Start the server. This method shoud be able to restart a server
         * stopped through stop(). Should throw a RuntimeException if the server
         * cannot be started
         */
        public void start();

        /**
         * Stop the server. This method should be able to stop server started
         * through start(). Should throw a RuntimeException if the server cannot
         * be stopped
         */
        public void stop();
