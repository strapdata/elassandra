...
public class ElassandraDaemon extends CassandraDaemon {
    ...
    private static ElassandraDaemon instance = new ElassandraDaemon();
    ...
    public void activate(boolean addShutdownHook, Tuple<Settings, Environment> tuple) {
        ...
        super.setup(); // start bootstrap CassandraDaemon and call beforeRecover()+beforeBootstrap() to activate ElasticSearch and wait for
                       // local shards
    ...
    public void beforeRecover() {
        try {
            KSMetaData ksMetaData = Schema.instance.getKSMetaData(ElasticSchemaService.ELASTIC_ADMIN_KEYSPACE);
            if (ksMetaData != null) {
    ...
    private void startElasticSearch() {
        instance.buildNode(addShutdownHook, tuple); // Initialize ElasticSearch by reading stored metadata.
    ...
    private void buildNode(boolean addShutdownHook, Tuple<Settings, Environment> tuple) {
        ...
        nodeBuilder.clusterName(DatabaseDescriptor.getClusterName()).data(true)
