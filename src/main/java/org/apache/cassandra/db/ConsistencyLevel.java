...
public enum ConsistencyLevel
{
    ...
    {
        return (keyspace.getReplicationStrategy() instanceof IDistributedReplicationStrategy)
             ? (((IDistributedReplicationStrategy) keyspace.getReplicationStrategy()).getReplicationFactor(dc) / 2) + 1
             : quorumFor(keyspace);
    }
    ...
    public int blockFor(Keyspace keyspace)
    {
        switch (this)
        {
            ...
            case EACH_QUORUM:
                if (keyspace.getReplicationStrategy() instanceof IDistributedReplicationStrategy)
                {
                    IDistributedReplicationStrategy strategy = (IDistributedReplicationStrategy) keyspace.getReplicationStrategy();
                    int n = 0;
    ...
    private void requireNetworkTopologyStrategy(String keyspaceName) throws InvalidRequestException
    {
        ...
        if (!(strategy instanceof IDistributedReplicationStrategy))
