...
public class CassandraDiscovery extends AbstractLifecycleComponent implements Discovery, IEndpointStateChangeSubscriber {
    ...
    private final AtomicReference<MetaDataVersionListener> metaDataVersionListener = new AtomicReference<>(null);
        ...
        synchronized (clusterGroup) {
            ...
            for (UntypedResultSet.Row row : executeInternal("SELECT peer, data_center, rack, rpc_address, host_id from system." + SystemKeyspace.PEERS)) {
                ...
                if ((!peer.equals(localAddress)) && (rpc_address != null) && (localDc.equals(datacenter))) {
                    ...
                    DiscoveryNode dn = new DiscoveryNode(buildNodeName(peer), host_id, new TransportAddress(rpc_address, publishPort()), attrs, CASSANDRA_ROLES, Version.CURRENT);
                ...
                if (vv != null) {
                    ...
                    if (dn == null) {
                        ...
                        dn = new DiscoveryNode(buildNodeName(internal_address), hostId.toString(), 
                                new TransportAddress(rpc_address, publishPort()), attrs, CASSANDRA_ROLES, Version.CURRENT);
                        dn.status(status);
        ...
        if (isLocal(endpoint)) {
            ...
            if (dn == null) {
                ...
                dn = new DiscoveryNode(buildNodeName(internal_address), 
                        hostId.toString(), 
                        new TransportAddress(rpc_address, publishPort()), 
    ...
    public void notifyMetaDataVersionAckListener(EndpointState endPointState) {
        MetaDataVersionListener listener = this.metaDataVersionListener.get();
        if (listener == null)
            return;
    ...
    public class MetaDataVersionListener  {
        ...
        public MetaDataVersionListener(long version, Discovery.AckListener ackListener, ClusterState clusterState) {
        ...
        public boolean await(long timeout, TimeUnit unit) {
            ...
            CassandraDiscovery.this.metaDataVersionListener.set(null);
            return done;
        }
    ...
    private boolean isNormal(DiscoveryNode node) {
        // endpoint address = C* RPC broadcast address = Elasticsearch publish_host
        EndpointState state = Gossiper.instance.getEndpointStateForEndpoint(node.getInetAddress());
        if (state == null) {
            logger.warn("Node endpoint address=[{}] name=[{}] state not found", node.getName(), node.getInetAddress());
            return false;
        }
        ...
        if (isMember(endpoint)) {
            ...
            case STATUS:
                ...
                if (isNormal(epState)) {
                    ...
                    if (this.metaDataVersionListener.get() != null) {
    ...
    public synchronized void publish(ClusterChangedEvent clusterChangedEvent, Discovery.AckListener ackListener) {
        ...
        MetaDataVersionListener metaDataVersionListerner = null;
        try {
            ...
            if (!newClusterStateMetaDataString.equals(previousClusterStateMetaDataString) && !newClusterState.blocks().disableStatePersistence() && clusterChangedEvent.peristMetaData()) {
                ...
                if (presistedMetadata && newClusterState.nodes().getSize() > 1) {
                    ...
                    metaDataVersionListerner = new MetaDataVersionListener(newClusterState.metaData().version(), ackListener, newClusterState);
                }
                try {
